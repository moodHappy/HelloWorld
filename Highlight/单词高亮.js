...
// @version      1.6 // Increment version for voice change
// @description  高亮页面指定单词。点击时，同时查询AI解释并直接朗读该单词（使用美国Eric语音）。AI解释弹窗出来后，再显示发音按钮。使用显眼的红色下划线，带简单缓存。支持移动端双击关闭弹窗。
// @author       You
// @match        *://*/*
// @grant        GM_addStyle
...
(function () {
    ...
    const aiCache = {}; // Object to store word -> AI explanation mapping

    function getCachedExplanation(word) {
        return aiCache[word.toLowerCase()]; // Cache keys are lowercase
    }

    function setCachedExplanation(word, explanation) {
        aiCache[word.toLowerCase()] = explanation; // Store explanation
    }
    ...
    let voice = 'en-US-EricNeural'; // 修改为美国男声 Eric
    ...
    // Added isSuccess flag to control display of word-info
    function showModal(content, word = '', isSuccess = false) {
        ...
        if (!modal || !wordToReadSpan || !aiBody || !playButton || !wordInfoDiv) {
    ...
    function callAI(word) {
        const cachedExplanation = getCachedExplanation(word);

        if (cachedExplanation) {
            console.log(`Cache hit for "${word}". Displaying cached explanation.`);
            // Pass true for isSuccess as we have a cached explanation
            showModal(cachedExplanation, word, true);
            ...
            GM_xmlhttpRequest({
                ...
                onload: res => {
                    try {
                        ...
                        if (reply) {
                            setCachedExplanation(word, reply); // Store the successful response in cache
                            resolve(reply);
                        } else {
                            reject("AI 返回内容为空。");
                        }
        ...
        sendRequest(primaryAI)
            .then(result => showModal(result, word, true)) // Pass true for success
            .catch(primaryErr => {
