...
// ==UserScript==
// @name         蓝色（支持词形变化）并标记已掌握单词（双击弹窗确认）
// @namespace    https://greasyfork.org/zh-TW
// @version      5.3
// @description  给网页关键词及其词形变化改变成蓝色，双击弹窗确认标记为已掌握，支持导出和导入已掌握的单词
...
(function() {
    ...
    // 定义十个关键词数组
    var keywords1 = ['on', 'evacuated', 'jump'];
    var keywords2 = ['speak', 'evacuat', 'read'];
    var keywords3 = ['are', 'been', 'study'];
    ...
    // 双击关键词时，弹出确认框
    function showConfirmationDialog(word) {
        var confirmed = window.confirm(`你确定标记 "${word}" 为已掌握单词吗？`);
        if (confirmed) {
            markAsMastered(word);
        }
    }

    // 标记单词为已掌握并保存到 localStorage
    ...
    function replaceKeywords(node) {
        ...
        for (var i = 0; i < allKeywords.length; i++) {
            ...
            text = text.replace(wordFormsRegex, function(match) {
                return `<span style="color:${randomColor()};cursor:pointer;" ondblclick="showConfirmationDialog('${match}')">${match}</span>`;
            });
        }
    ...
    // 将 showConfirmationDialog 暴露到全局作用域，以便双击事件可以调用
...
(function() {
    ...
    // 定义十个关键词数组
    var keywords1 = ['on', 'found', 'jump'];
    var keywords2 = ['speak', 'write', 'read'];
    var keywords3 = ['are', 'sleep', 'study'];
    var keywords4 = ['work', 'travel', 'sing'];
    var keywords5 = ['cook', 'dance', 'shop'];
    var keywords6 = ['read', 'swim', 'laugh'];
    var keywords7 = ['think', 'listen', 'walk'];
