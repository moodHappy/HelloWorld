...
(function () {
    ...
    function traverseAndHighlight(node, keywords, color) {
        const regexList = keywords.map(buildRegex); // 为每个单词构建正则
            ...
            loadKeywords(urlGroup1, (keywords) => {
                ...
                loadKeywords(urlGroup2, (keywords) => {
                    ...
                    loadKeywords(urlGroup3, (keywords) => {
                        ...
                        // 高亮删除标记（优先处理）
                        traverseAndHighlight(document.body, deleteKeywords, colors.deleted);

                        // 按分组高亮关键词
                        traverseAndHighlight(document.body, group1Keywords.filter(k => !deleteKeywords.includes(k)), colors.group1);
                        traverseAndHighlight(document.body, group2Keywords.filter(k => !deleteKeywords.includes(k)), colors.group2);
                        traverseAndHighlight(document.body, group3Keywords.filter(k => !deleteKeywords.includes(k)), colors.group3);
                    });
                });
            });
        });
