...
(function () {
    ...
    // 定义已掌握关键词数组
    var masteredKeywords = ['run', 'speak', 'eat', 'work', 'watch']; // 你可以在这里手动添加已掌握的关键词
  ...
  'recommendation', 'constitutional', 'industrialized', 'secretary-general'
    ...
    function createResultsBox() {
        ...
        box.id = 'wordFrequencyBox';
        box.style.position = 'fixed';
        box.style.bottom = '60px'; // 保留空间给切换按钮
        box.style.left = '10px';
        box.style.right = '10px';
        box.style.maxHeight = '50%';
        box.style.overflowY = 'auto';
    ...
    function displayWordCount(wordCount) {
        ...
        wordCountBox.innerHTML = ''; // 清空之前的结果
        const sortedWords = Object.keys(wordCount).sort((a, b) => wordCount[b] - wordCount[a]);
    ...
    toggleButton = createToggleButton(); // 向页面添加切换按钮
    document.addEventListener('click', handleDocumentClick); // 监听点击事件
