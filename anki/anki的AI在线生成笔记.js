...
window.onload = function() {
  // 获取Front字段的文本内容
  const frontText = document.querySelector('.front-text') ? document.querySelector('.front-text').textContent.trim() : '';  // 如果没有 Front 内容，直接退出
  if (!frontText) {
  ...
  // 在Anki中使用字段存储缓存数据
  const cacheKey = `analysis_cache_${frontText}`; // 使用文本作为缓存的key
...
if (!frontText) {  
    ...
    if (notesField && notesField.textContent.trim()) {
        // 如果“笔记”字段已经有内容，则不执行在线分析
        console.log('已存在笔记内容，跳过在线分析');
    } else {
        // 获取缓存内容
        const cachedAnalysis = localStorage.getItem(cacheKey);
        if (cachedAnalysis) {  
