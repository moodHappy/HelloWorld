...
下文中的can't have been是 can't have been driving的省略式，
在意思上与must have been driving正相反。 
...
must与can't不仅能表达对现在和将来的猜测和推断，
而且能表达对过去的猜测和推断——这时可用 must have been表示肯定的推断，
用 can't have been（而不是mustn't have been）来表示否定的推断，
如后面跟进行时则表示对过去正在进行的动作的猜测。
请看例句： 
