<div id="version-info" style="display: none;">版本号：2025.9.4</div>
        ...
        idiomWords.forEach(idiom => {
            ...
            if (idiomWordsArray.length >= 2) {
                ...
                for (let j = 0; j < exampleWords.length; j++) {
                    const currentExampleBaseForms = getWordBaseForms(exampleWords[j], wordForms);
                    // 检查当前例句词的任何一个词根是否与当前习语词的词根匹配
                    const isMatch = currentExampleBaseForms.some(baseForm => {
                        return baseFormIdiomWords[currentIdiomWordIndex] === baseForm;
                    });
                    
                    if (currentIdiomWordIndex === 0 && isMatch) {
                        startIndex = j;
                        currentIdiomWordIndex++;
                    } else if (startIndex !== -1 && currentIdiomWordIndex < baseFormIdiomWords.length && isMatch) {
                        currentIdiomWordIndex++;
                    }
