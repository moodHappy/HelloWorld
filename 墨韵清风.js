...
if (exampleSentence.innerHTML.trim() !== "") {
    ...
    urls.forEach(url => {
        fetch(url)
            ...
            .then(data => {
                // 将获取到的文本按行分割，并去除首尾空格后添加到Set中
                data.split('\n').forEach(word => {
                    const trimmedWord = word.trim();
                    if (trimmedWord) {
                        wordsToHighlight.add(trimmedWord);
                        if (url.includes('one.txt')) {
                            blueWords.add(trimmedWord);
                        } else if (url.includes('two.txt')) {
                            redWords.add(trimmedWord);
                        } else if (url.includes('idiom.txt')) {
                            idiomWords.add(trimmedWord);
                        }
                    }
                });
            })
            .catch(error => {
                console.error(`获取URL ${url} 时出错:`, error);
            })
            .finally(() => {
                pendingRequests--;
                // 当所有请求都完成后，开始处理例句
    ...
    function getStem(word) {
        return verbStems[word.toLowerCase()] || word.toLowerCase();
    }
    ...
    function processExampleSentence() {
        ...
        idiomWords.forEach(idiom => {
            const idiomWordsArray = idiom.split(' ').filter(word => !['a', 'an', 'the', 'in', 'on', 'at', 'of', 'for', 'with'].includes(word.toLowerCase()));
            const stemmedIdiomWords = idiomWordsArray.map(word => getStem(word));
            
            // 查找习语在例句中的位置
            let startIndex = -1;
            let currentIdiomWordIndex = 0;
            for (let j = 0; j < exampleWords.length; j++) {
                const currentExampleStem = getStem(exampleWords[j]);
                if (currentIdiomWordIndex === 0 && currentExampleStem === stemmedIdiomWords[0]) {
                    startIndex = j;
                    currentIdiomWordIndex++;
                } else if (startIndex !== -1 && currentIdiomWordIndex < stemmedIdiomWords.length && currentExampleStem === stemmedIdiomWords[currentIdiomWordIndex]) {
                    currentIdiomWordIndex++;
                }
                
                if (currentIdiomWordIndex === stemmedIdiomWords.length) {
                    const endIndex = j;
        ...
        // 普通单词匹配逻辑 (在习语匹配之后执行)
        const allWordsRegex = new RegExp(`\\b(${Array.from(wordsToHighlight).join('|')})(?:s|es|d|ed|ing|en)?\\b`, 'gi');
        
        newHtml = newHtml.replace(allWordsRegex, (match, baseWord) => {
             // 检查该匹配是否已经是已高亮的习语的一部分
             if (newHtml.includes(`<span class="green-word">${match}</span>`)) {
                 return match; // 不再重复处理
             }

             // 检查原始词是否在蓝色集合中
             if (blueWords.has(baseWord.toLowerCase())) {
                 return `<span class="blue-word">${match}</span>`;
             } else if (redWords.has(baseWord.toLowerCase())) {
                 return `<span class="red-word">${match}</span>`;
             } else {
...
if (exampleSentence.innerHTML.trim() !== "") {
    ...
    function processExampleSentence() {
        ...
        // 构建一个正则表达式，匹配所有需要高亮的单词，并考虑单词边界
        const allWordsRegex = new RegExp(`\\b(${Array.from(wordsToHighlight).join('|')})\\b`, 'gi');
        
        let newHtml = originalHtml.replace(allWordsRegex, (match, baseWord) => {
            if (idiomWords.has(baseWord.toLowerCase())) {
                return `<span class="green-word">${match}</span>`;
            } else if (blueWords.has(baseWord.toLowerCase())) {
                return `<span class="blue-word">${match}</span>`;
            } else {
                return `<span class="red-word">${match}</span>`;
            }
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (cached) return insertAnalysis(cached, '来源：KIMI');
  ...
  fetchFromAPI(mainApi, '<br><br>**来源**：KIMI')
  .catch(() => alert('无法连接到KIMI服务。'));
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const cacheKey = `analysis_cache_${frontText}`;
  const cached = localStorage.getItem(cacheKey);
  if (cached) return insertAnalysis(cached, '来源：OpenRouter (Deepseek)');

  const mainApi = {
    url: 'https://openrouter.ai/api/v1/chat/completions',
    key: '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',
  ...
  fetchFromAPI(mainApi, '<br><br>**来源**：OpenRouter (Deepseek)')
  .catch(() => alert('无法连接到OpenRouter (Deepseek)AI服务。'));
