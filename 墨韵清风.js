...
<script>
  ...
  function getRandomUSVoice() {
    return usVoices[Math.floor(Math.random() * usVoices.length)];
  }
  ...
  const voiceMap = {
    ...
    "he": "he-IL-AvriNeural",
    "": getRandomUSVoice() // 如果语种为空，则随机选择一个美国语音
  };
  ...
  function playTTS(text, audioId, callback) {
    ...
    if (useAlternativeTTS) {
      ...
      src = `https://ms-ra-forwarder-for-ifreetime-2.vercel.app/api/aiyue?${queryString}`;
    } else {
...
<div id="version-info" style="display: none;">version: 2.0.0 (Nebula)</div>
