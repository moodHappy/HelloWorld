...
if (exampleSentence.innerHTML.trim() !== "") {
    ...
    .then(([oneTxt, twoTxt, idiomTxt, wordForms]) => {
        ...
        function getWordBaseForm(word) {
            ...
            const slice = (len) => lowerCaseWord.slice(0, len);
            
            // 规则 #1: 比较级和最高级
            if (len > 3) {
                if (endsWith('er')) {
                    const base = slice(-2);
                    if (base.length > 0 && base[base.length - 1] === lowerCaseWord[lowerCaseWord.length - 3]) return slice(-3);
                    if (endsWith('ier')) return slice(-3) + 'y';
                    return base;
                }
                if (endsWith('est')) {
                    const base = slice(-3);
                    if (base.length > 0 && base[base.length - 1] === lowerCaseWord[lowerCaseWord.length - 4]) return slice(-4);
                    if (endsWith('iest')) return slice(-4) + 'y';
                    return base;
                }
                if (endsWith('r') && len > 1) { // large -> larger
                    const base = slice(-1);
            ...
            if (len > 3 && endsWith('ing')) {
                const base = slice(-3);
                if (base.length > 0 && base[base.length - 1] === lowerCaseWord[lowerCaseWord.length - 4]) return slice(-4);
                if (base.length > 0 && !endsWith('e') && (lowerCaseWord.charAt(lowerCaseWord.length - 4) !== 'e')) {
                    // This is a more robust check for 'e' removal.
                }
                if (endsWith('ying')) return slice(-3) + 'e';
                return base;
            }
            
            // 规则 #3: 动词的过去式和过去分词
            if (len > 2) {
                // 处理以 'e' 结尾加 'd' 的情况 (mumble -> mumbled)
                if (endsWith('d')) {
                    const base = slice(-1);
                    if (base.endsWith('e')) return base;
                }
                // 处理常规 'ed' 结尾的情况 (work -> worked)
                if (endsWith('ed')) {
                    const base = slice(-2);
                    // 双写辅音（如 stopped）
                    if (base.length > 0 && base[base.length - 1] === lowerCaseWord[lowerCaseWord.length - 3]) return slice(-3);
                    // y 变 i 加 ed（如 studied）
            ...
            if (len > 1 && endsWith('s')) {
                ...
                if (endsWith('es')) return slice(-2);
                if (!endsWith('ss') && !endsWith('is') && len > 1) return slice(-1);
            }
