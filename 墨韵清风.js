...
if (exampleSentence.innerHTML.trim() !== "") {
    ...
    .then(([oneTxt, twoTxt, idiomTxt, wordForms]) => {
        ...
        function getWordBaseForm(word) {
            ...
            if (wordForms[lowerCaseWord]) return wordForms[lowerCaseWord];
            ...
            if (len > 3) {
                if (endsWith('er')) {
                    ...
                    const base = slice(2);
                    if (endsWithCVC(base)) return slice(3);
                    return base;
                }
                if (endsWith('est')) {
                    if (endsWith('iest')) return slice(4) + 'y';
                    const base = slice(3);
                    if (endsWithCVC(base)) return slice(4);
                    return base;
                }
            }

            // 动词过去式 / 过去分词
            if (len > 2 && endsWith('ed')) {
                if (endsWith('ied')) return slice(3) + 'y';
                let base2 = slice(2);
                if (endsWithCVC(base2)) return slice(3);
                if (base2.endsWith('e')) return base2;
                return base2;
            }
            ...
            // 名词复数
            if (len > 1 && endsWith('s')) {
                if (endsWith('ies') && !'aeiou'.includes(lowerCaseWord[len-4])) return slice(3) + 'y';
                if (endsWith('es')) {
        ...
        function isWordInSet(word, set) {
            const base = getWordBaseForm(word);
            return set.has(word.toLowerCase()) || set.has(base);
        }
