(function() {
    ...
    console.log('EnLight Core Logic Loaded');
    ...
    // --- 样式变量注入 (核心修改：适配外部 CSS) ---
    function applyUserStyles() {
        const c = getConfig();
        const root = document.documentElement;
        
        // 基础变量
        root.style.setProperty('--el-font-size-ratio', (parseInt(c.style.fontSizeRatio) || 100) / 100);
        root.style.setProperty('--el-line-height', c.style.lineHeight);
        root.style.setProperty('--el-text-color', c.style.color);
        root.style.setProperty('--el-margin-top', c.style.marginTop);

        // 主题变量映射
        const themes = {
            card: { bg: '#f7f9fa', borderL: '3px solid #007AFF', pad: '6px 10px', rad: '4px', shadow: '0 1px 3px rgba(0,0,0,0.05)', border: 'none', borderB: 'none', style: 'normal' },
            minimal: { bg: 'transparent', borderL: 'none', pad: '2px 0', rad: '0', shadow: 'none', border: 'none', borderB: 'none', style: 'italic', color: '#555' },
            dashed: { bg: '#fff', borderL: 'none', pad: '6px 10px', rad: '6px', shadow: 'none', border: '1px dashed #999', borderB: 'none', style: 'normal' },
            underline: { bg: 'transparent', borderL: 'none', pad: '2px 0 6px 0', rad: '0', shadow: 'none', border: 'none', borderB: '1px solid #ddd', style: 'normal' },
            dark: { bg: '#2c2c2e', borderL: '3px solid #FF9500', pad: '6px 10px', rad: '4px', shadow: 'none', border: 'none', borderB: 'none', style: 'normal' }
        };

        const t = themes[c.style.theme] || themes.card;
        root.style.setProperty('--el-bg-color', t.bg);
        root.style.setProperty('--el-border-left', t.borderL);
        root.style.setProperty('--el-padding', t.pad);
        root.style.setProperty('--el-border-radius', t.rad);
        root.style.setProperty('--el-box-shadow', t.shadow);
        root.style.setProperty('--el-border', t.border);
        root.style.setProperty('--el-border-bottom', t.borderB);
        root.style.setProperty('--el-font-style', t.style);
        
        if(c.style.theme === 'dark') {
             root.style.setProperty('--el-text-color', '#e5e5e5');
        } else if (c.style.theme === 'minimal') {
             root.style.setProperty('--el-text-color', '#555');
        }
    }
    ...
    function ensureNlp() {
        ...
        isNlpLoading = true;
        // compromise 已经通过 @require 加载，这里只是为了保险起见或者如果 CDN 失败
        if (window.nlp) { nlpReady = true; isNlpLoading = false; return Promise.resolve(); }
        
        return Promise.reject("NLP library not loaded via @require");
    }
    ...
    // ==========================================
    // 2. 查词弹窗 CSS (Shadow DOM 专用)
    // ==========================================
    // ShadowDOM 的样式必须内联，无法轻易使用外部 CSS（除非 fetch 注入），所以保持字符串
    const POPUP_CSS = `
        :host { all: initial; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; z-index: 2147483647; }
        #custom-dict-popup {
        ...
        .g-tag {
            background: #f7bf34; color: #fff; padding: 1px 6px; border-radius: 4px;
            font-size: 11px; font-weight: bold; display: inline-block; line-height: 1.4;
        }
        .g-tag.collins { background: #ff9c38; }
    ...
    const IGNORE_SELECTORS = [
        ...
        '[class*="Metadata"]', '[class*="Byline"]', '[class*="Contributor"]',
        '[class*="Copyright"]', '[class*="ImageMessage"]'
    ];
    ...
    async function processTranslationQueue() {
        ...
        try { 
            ...
            // 动态设置行内样式以覆盖 CSS 默认值，如果需要的话
            loadingDiv.style.fontSize = `calc(${computed.fontSize} * var(--el-font-size-ratio))`;
    ...
    function renderTranslation(targetElement, translatedText, isCached) {
        ...
        try {
            ...
            if (rect.width > 0 && rect.width < window.innerWidth * 0.95) {
                div.style.maxWidth = `${rect.width}px`; 
            }
            
            div.style.marginLeft = window.getComputedStyle(targetElement).marginLeft;
            div.style.paddingLeft = window.getComputedStyle(targetElement).paddingLeft;
            
            if (originalFontSize) {
                // 使用 CSS 变量计算比例
                div.style.fontSize = `calc(${originalFontSize}px * var(--el-font-size-ratio))`;
            }
    ...
    function parseYoudaoHtml(html, originalWord) {
        ...
        return `
            ...
            ${ (phone || tagsHtml) ? `<div class="g-meta">${phone ? `<span class="g-phonetic">${phone}</span>` : ''}${tagsHtml}</div>` : '' }
            ${defsHtml}
        `;
    }
    ...
    function showToast(msg, dur=2000) {
        ...
        if (!toast) {
            toast = document.createElement('div'); toast.id = 'wh-toast';
            toast.style.cssText = "position:fixed;top:20px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.8);color:white;padding:8px 16px;border-radius:20px;font-size:14px;z-index:1000001;pointer-events:none;opacity:0;transition:opacity 0.3s;";
    ...
    // ==========================================
    // 8. 设置界面 (省略了部分重复代码，核心逻辑相同，注意样式变量)
    // ==========================================
    function openSettings() {
        ...
        m.style.cssText=`display:flex;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:1000000;align-items:center;justify-content:center;font-family:sans-serif;`;
            ...
            reader.onload = (event) => {
                try {
                    ...
                    if(parsed.urls && parsed.style) {
                        ...
                        showToast('配置导入成功，即将刷新');
                        setTimeout(() => location.reload(), 1000);
                    } else showToast('JSON 格式错误');
                } catch(ex) { showToast('JSON 解析失败'); }
            };
    ...
    GM_registerMenuCommand("🗑️ 清空翻译/词典缓存", () => IDB.clear().then(() => showToast('缓存已清空')));
